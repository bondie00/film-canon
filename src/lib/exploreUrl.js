 ...
 *
 * `country`, `director` and `genre` are repeatable on /explore, so each takes
 * either one name or an array (the world map selects a whole continent at once).
 */
const listOf = (value) => (value == null ? [] : Array.isArray(value) ? value : [value])

export function exploreUrl({ poll, top = null, country = null, director = null, genre = null, yearRange = null } = {}) {
  ...
  listOf(genre).forEach(name => params.append('genre', name))
