...
// Selection UI for the country visualizations, driven by a useCountrySelection()
// instance (`sel`). Split into two halves because they sit in different places:
// the quick filters go ABOVE the visualization and the search-and-add dropdown
// BELOW it, matching the bar chart's layout.

/** Top-N and per-continent quick filters. Renders above the visualization. */
export function CountryQuickFilters({ sel }) {
  const { activeContinents, activeButton, defaultCount, resetToTopN, selectContinent } = sel
...
/** Search-and-add country picker. Renders below the visualization. */
export function CountrySearchDropdown({ sel }) {
  const {
    selectedCountries, maxSelection,
    isDropdownOpen, pendingSelection, searchQuery, setSearchQuery, expandedContinents,
    filteredContinents,
    openDropdown, closeDropdown, toggleContinentExpanded,
